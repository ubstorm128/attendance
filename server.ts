...
const server = http.createServer(async (req: import("http").IncomingMessage, res: import("http").ServerResponse) => {
    ...
    if (req.method === 'POST' && pathname === '/api/mark') {
        const { enrollment, otp } = await readBody(req);
        const student = db.students[enrollment];
        if (!student) return send(res, 404, { error: 'register first' });
        const session = db.sessions.find(s => s.active && s.otp === otp);
        if (!session) return send(res, 401, { error: 'wrong or expired otp' });
    ...
    if (req.method === 'POST' && pathname === '/api/generate-otp') {
        const { teacherId, subject } = await readBody(req) as { teacherId?: string; subject?: string };
        ...
        const session: SessionRecord = {
            ...
            otp: String(nodeCrypto.randomInt(100000, 999999)),
            present: {},
        ...
        return send(res, 200, { otp: session.otp, sessionId: session.id });
    }

    if (req.method === 'POST' && pathname === '/api/close-session') {
        const { teacherId } = await readBody(req);
        const session = activeSessionFor(teacherId);
        if (session) { session.active = false; session.otp = null; save(); broadcastTo(teacherId, { type: 'closed' }); }
        return send(res, 200, { ok: true });
    }
